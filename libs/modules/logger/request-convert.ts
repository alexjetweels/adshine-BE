import { RequestType } from 'convert-pino-request-to-curl/utils/entity';
import { ValuesImportant } from 'libs/utils/enum';
import { hideImportantInformation } from 'libs/utils/util';

export class PinoRequestConverterCustomer {
  static getCurl(request: RequestType): string {
    ...
    Object.keys(request?.params).forEach(
      (p) => (params += `/${p}/${request.params[String(p)]}`),
    );
    ...
    const rawBody = hideImportantInformation(
      request?.raw?.body,
      ValuesImportant,
    );
